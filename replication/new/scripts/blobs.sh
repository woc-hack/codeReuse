...
join -t\; \
    - \
    <(zcat "data/blobs/b2tPFull${ver}$i.copied") | 
...
join -t\; -a1 \
    - \
    <(zcat "data/blobs/b2tPFull${ver}$i.copied" | awk '{print $1";1"}') | 
...
join -t\; \
    - \
    <(zcat "${dir}b2slfclFull${ver}$i.s") | 
gzip >"b2tPFull${ver}$i.FirstCopiedSize"
...
# copiedb2nb (ob2b) (probably just 14 is relevant!)
for i in {0..127}; do
    LC_ALL=C LANG=C join -t\; \
        <(zcat data/blobs/b2tPFullU14.copied) \
        <(zcat "${dir}ob2bFullU$i.s")
done |
~/lookup/lsort 50G -t\; |

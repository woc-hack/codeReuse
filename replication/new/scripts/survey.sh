...
for set in sTb bTb bTs sTs rand; do
    ...
    while read -r line; do
        ...
        count=$(grep "$ud" <"data/survey/initial/$set.counts" |
            sed 's| *||' | 
            cut -d' ' -f1)

        u=$(echo "$ud" | cut -d\; -f1)

        fcount=$(grep "$u" <"data/survey/initial/$set.fcounts" |
            sed 's| *||' | 
            cut -d' ' -f1)

        echo "$line;$type;$count;$fcount"
    done <"data/survey/initial/$set" >"data/survey/initial/$set.2"
done
...
for set in sTb bTb bTs sTs rand; do
    awk '{i+=1; if (i%100 == 1) print}' <"data/survey/initial/$set.counts" |
    sed 's| *||' |
    cut -d' ' -f2 |
    while read -r line; do
        ...
        type=$(grep "$u" <"data/survey/initial/$set.2" |
            grep "$d" |
            cut -d\; -f24 |
            ~/lookup/lsort |
            uniq -c |
        ...
        grep "$u" <"data/survey/initial/$set.2" |
        grep "$d" |
    ...
    c=$(wc -l <"data/survey/initial/$set.candidates")

    ~/lookup/lsort 10G -t\; -k24,24 <"data/survey/initial/$set.candidates" |
